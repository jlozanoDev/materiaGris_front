<template>
  <Modal :show="show" size="lg" iconClass="h-6 w-6 text-indigo-600" @close="close">
    ...
    <template #header>
      <h3 class="text-lg font-semibold text-slate-800 mb-4">{{ isNewUser ? 'Crear usuario' : 'Editar usuario' }}</h3>
    </template>
    ...
    <form @submit.prevent="onSave" class="space-y-4">
      ...
      <div class="mb-4">
        ...
        <template v-else>
          <input :value="email" readonly aria-readonly="true" type="email" class="form-input readonly-input" />
        ...
        <div class="space-y-2 max-h-40 overflow-y-auto">
          <label
            ...
            :class="[
              ...
              role.is_system ? 'opacity-60 cursor-not-allowed bg-slate-50' : 'cursor-pointer hover:bg-slate-50',
      ...
      <div class="mb-4">
        <div class="flex items-center justify-between mb-2">
          ...
          <button type="button" @click="resetPermissions" class="btn btn-ghost btn-sm">Restaurar permisos</button>
        </div>
        ...
        <div v-else class="space-y-3 max-h-60 overflow-y-auto">
          <div v-for="category in permissionsByCategory" :key="category.id" class="border rounded p-2">
            ...
            <div class="space-y-1">
              ...
              >
                ...
                <template v-if="isPermissionFromRole(perm.id)">
                  ...
                  <span :class="getEffectiveGrant(perm.id) === 1 ? 'text-green-600' : 'text-red-500'">
      ...
      <div class="mb-4 p-3 bg-slate-50 rounded">
        <label class="block text-sm font-medium text-slate-600 mb-2">Permisos Efectivos (resultado)</label>
        <div class="flex flex-wrap gap-1">
          ...
          <span v-if="Object.keys(effectivePermissionsMap).length === 0" class="text-sm text-slate-400">
            Sin permisos
          </span>
        </div>
      </div>
      ...
      <div v-if="overridesToRemove.length > 0" class="p-3 bg-amber-50 border border-amber-200 rounded">
        <p class="text-sm text-amber-700">
          Se eliminarán {{ overridesToRemove.length }} permisos individuales al asignar los roles seleccionados.
        </p>
        <div class="mt-2 flex flex-wrap gap-2">
          <span v-for="item in overridesToRemove" :key="'override-'+String(item)" class="badge badge--danger text-xs">
      ...
      <div class="flex justify-end gap-3">
        ...
        <button type="submit" :disabled="!canSave" class="btn btn-primary disabled:opacity-50">Guardar</button>
      </div>
    </form>
  </Modal>
</template>
...
const { permissions: allPermissionsList, fetchPermissions, loading: loadingPermissions } = usePermissions()
...
function isPermissionFromRole(permissionId) {
  ...
  const fromUserRole = userPermissionsList.value.some(p => p.permission_id === permissionId && p.origin === 'role')
...
function getEffectiveGrant(permissionId) {
  ...
  if (selectedPermissions.value[permissionId] !== undefined && selectedPermissions.value[permissionId] !== 0) {
...
watch(() => allPermissionsList.value, (perms) => {
