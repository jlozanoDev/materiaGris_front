  ...
  <Modal :show="show" size="max-w-5xl" iconClass="h-6 w-6 text-indigo-600" @close="onClose">
    ...
    <template #header>
      <div class="flex items-center justify-between mb-4">
        ...
        <span v-if="editing" class="inline-flex items-center px-2 py-1 rounded-2xl text-xs font-medium bg-yellow-100 text-yellow-800">Editando...</span>
      </div>
    </template>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div class="md:col-span-2">
        <div class="flex items-center justify-between mb-3">
          <input v-model="globalFilter" placeholder="Buscar..." aria-label="Buscar direcciones" class="form-input md:w-1/3" />
        </div>

        <UiVuetifyDataTable class="addresses-table" :value="localAddresses" dataKey="id" :filters="filters" :globalFilterFields="['alias','street','number','postal_code','mobile_phone']" :columns="columns">
            ...
            <div class="px-3 py-2 text-right">
              <div class="flex items-center justify-end gap-2">
                <button @click="startEdit(data)" aria-label="Editar" class="icon-action group">
                  ...
                  <span class="absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap bg-slate-800 text-white text-xs py-1 px-2 rounded opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity">Editar</span>
                </button>
                <button @click="confirmDelete(data)" aria-label="Eliminar" class="icon-action icon-action--danger group">
                  <i class="pi pi-trash h-4 w-4 transition-colors duration-150"></i>
                  <span class="absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap bg-slate-800 text-white text-xs py-1 px-2 rounded opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity">Eliminar</span>
                </button>
              </div>
            </div>
          </template>
        ...
        <div v-if="editing" class="w-full">
          <div class="form-row">
            ...
            <div class="flex items-center gap-3">
              ...
              <h4 class="font-medium text-slate-800">{{ isNew ? 'Nueva dirección' : 'Editando dirección' }}</h4>
            </div>
            ...
            <div class="flex items-center gap-3">
              <button type="button" @click="form.is_primary = !form.is_primary" :aria-pressed="form.is_primary" :class="['toggle', form.is_primary ? 'toggle--on' : '']">
...
const props = defineProps({ show: { type: Boolean, default: false }, addresses: { type: Array, default: () => [] } })
...
function startNew() {
  editing.value = true
  isNew.value = true
  form.value = { id: Date.now(), alias: '', street: '', number: '', postal_code: '', mobile_phone: '', landline_phone: '', contact_email: '', is_primary: false }
}
...
function cancelEdit() { editing.value = false; isNew.value = false; form.value = {} }
