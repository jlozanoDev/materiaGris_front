          ...
          <template v-else>
            <div class="modal-header">
              <div class="flex items-center gap-3">
                <div v-if="$slots.icon" class="flex-shrink-0 flex items-center modal-icon" :class="iconClass">
                  <slot name="icon" />
                </div>
                <div v-else-if="icon" class="flex-shrink-0 flex items-center modal-icon" v-html="icon" :class="iconClass" />
...
const sizeClass = computed(() => {
  const map = { sm: 'max-w-sm', md: 'max-w-md', lg: 'max-w-3xl', xl: 'max-w-4xl', full: 'max-w-full' }
...
.modal-icon svg { display: block; vertical-align: middle; }
.modal-icon svg { height: 1.5rem; width: 1.5rem; }
