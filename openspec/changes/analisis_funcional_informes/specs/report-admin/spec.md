...
The system MUST display a paginated, filterable table of patient reports for users with `report.view`.

(Previously: described as "admin users" without specifying the permission slug.)

#### Scenario: User with `report.view` accesses report listing

- GIVEN a user with permission `report.view`
...
#### Scenario: Route guard denies user WITHOUT `report.view`

- GIVEN a user WITHOUT permission `report.view`
- WHEN navigating to the reports listing URL
- THEN the route guard checks `meta.permissions: 'report.view'`
- AND `authStore.hasPermission('report.view')` returns false
- AND redirects to Dashboard

#### Scenario: User with `report.view` filters by status
...
#### Scenario: User with `report.view` filters by patient
...
#### Scenario: User with `report.view` filters by date range
...
#### Scenario: User with `report.view` filters by template
...
The system MUST render a read-only view of any report for users with `report.view`, regardless of report status.

#### Scenario: User with `report.view` views a draft report

- GIVEN a user with `report.view` clicks "Ver" on a draft report
...
#### Scenario: User with `report.view` views a signed or closed report

- GIVEN a user with `report.view` clicks "Ver" on a signed or closed report
- WHEN the report viewer opens
- THEN the form renders as read-only
- AND the PDF download button is visible ONLY if user also has `report.download-pdf`
- AND the signature canvas displays the stored signature image

#### Scenario: User WITHOUT `report.view` cannot access report viewer

- GIVEN a user WITHOUT `report.view`
- WHEN navigating to a report viewer URL directly (bypassing UI)
- THEN the route guard checks `meta.permissions: 'report.view'`
- AND redirects to Dashboard
- AND the report data is never fetched

#### Scenario: User with `report.view` cannot access edit URL

- GIVEN a user with `report.view` but WITHOUT `report.edit`
- WHEN navigating directly to a report edit URL
- THEN the route guard checks `meta.permissions` (which requires `report.edit`)
- AND redirects to Dashboard or renders read-only

### Requirement: PDF Download

The system MUST provide PDF download for signed and closed reports, gated by `report.download-pdf`.

#### Scenario: User with `report.download-pdf` downloads PDF

- GIVEN a user with `report.view` AND `report.download-pdf`
...
#### Scenario: User WITHOUT `report.download-pdf` sees no download action

- GIVEN a user with `report.view` but WITHOUT `report.download-pdf`
- AND a signed or closed report in viewer mode
...
- WHEN a user with `report.download-pdf` views it
...
- GIVEN a user with `report.download-pdf` and a signed report
...
- GIVEN the reports listing route with `meta: { requiresAuth: true, permissions: 'report.view' }`
- WHEN a user WITHOUT `report.view` navigates to the route
- THEN the `beforeEach` guard evaluates `authStore.hasPermission('report.view')`
- AND redirects to Dashboard

#### Scenario: Route guard for report viewer

- GIVEN the report viewer route with `meta: { requiresAuth: true, permissions: 'report.view' }`
- WHEN a user WITHOUT `report.view` navigates to the route
...
- WHEN the current user lacks `report.view`
...
- THEN the "Descargar PDF" button renders only if `authStore.hasPermission('report.download-pdf')`
...
- THEN it verifies `authStore.hasPermission('report.view')` as a defense-in-depth check
...
- WHEN a user with `report.view` loads the report listing
...
- Route and sidebar gated by `report.view` permission slug
...
- All reports render read-only for users with `report.view` (no edit controls)
- PDF download button visible only when `report.download-pdf` is held AND report is signed/closed
